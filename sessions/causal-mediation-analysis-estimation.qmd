...
nhanes <- nhanes %>%
  select(id = seqn, w1 = age, w2 = gender, w3 = education, a = total_meat, y = plasma_fasting_glucose, m = serum_c_reactive_protein) %>%
  na.omit() %>%
  mutate(
...
Here, we see that higher red meat intake is associated with lower CRP
levels.
...
In this example we will set it to the median `r median_m_log` which is a
CRP at `r median_m` which is a low level.
...
6.6% of the association between red meat and blood glucose is mediated
by M, inflammation. Not a lot.
...
Under the above mentioned assumptions, we can also calculate the
estimate using the G-computation approach.

The approach here is more manual but we also have more flexibility in
the models with use.

We will test this out using the Framgingham dataset. Our research
question is:

> Does obesity lead to higher CVD risk by increasing blood pressure?

We load the data, rename the variables, make a few categorical variables
...
Based on the outcome regression model, we can predict Y under different
scenarios. We first make a function that predict the outcome with
different inputs for A and M. Then we use the function to predict the
out with different combinations of A and M.
...
We now make a new function that based on the values of A, run the
prediction models of M. This is the nested counterfactual.

```{r}
# estimates of Pr(M = m | A = a, W = W_i), predict m based on a
get_Pm_a_Wi <- function(full_data, med_fit, A, M) {
  data_Aa_Wi <- full_data
  data_Aa_Wi$a <- A
...
Now, you can freely calculate the natural direct effect and indirect
effect:
...
E_Y1M1-E_Y0M1 \### Natural indirect effect E_Y1M1-E_Y1M0 \## 3-way and
4-way decomposition of effects

### R package example for causal mediation analysis - CMAverse-package

Test of other package #install devtools, go to github website to install
the CMAverse, Rtools
...
res_rb <- cmest(
  data = df, model = "gformula", outcome = "y", exposure = "a",
  mediator = "m", basec = c("w"), EMint = TRUE,
  mreg = list("linear"), yreg = "linear",
  astar = 5, a = 7, mval = list(5),
  estimation = "paramfunc", inference = "delta"
