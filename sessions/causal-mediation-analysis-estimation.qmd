...
nhanes <- nhanes %>%
  select(id = seqn, w1 = age, w2 = gender, w3 = education, a = x, y, m) %>%
  na.omit() %>%
  mutate(
    a = a + 50, 
    y = y - 50,
...
In this example we will set it to the median which is a CRP at
`r median_m` which is a low level.
...
### Why do we need g-computation approach
...
We will test this out using the Framingham dataset. Our research
question is:
...
res_rb <- cmest(
  ...
  estimation = "paramfunc", inference = "boot"
)
