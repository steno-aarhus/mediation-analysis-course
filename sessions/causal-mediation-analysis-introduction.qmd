...
analysis. For the mediation analysis, this introduction targets the
controlled direct effect, the indirect and direct effect as overall
concepts. In the next sessions, we will dive further into additional
estimands and decompositions of the effects.
...
had everyone, counter to the fact, recieved treatment a = 1)

$Y^{a=0}$ = Counterfactual outcome under treatment a (i.e., the outcome
had everyone, counter to the fact, recieved treatment a = 0)
:::
...
in the treatment effect in each group. Or put more formally E\[Y\^{a=1}
= 1\] - E\[Y\^{a=0} = 1\]
...
A DAG contains nodes and arrows. It cannot be circular. Fig. 2 shows the
different types of variables in a DAG and rules for paths
(ref:Tennant2021).

```{r, echo = FALSE, fig.cap="Fig. 2. Components of a DAG, including the common types of variables and paths. From Tennant et al. 2021", out.width = '100%'}
...
by: - outlining all confounding assumptions needed - handling
non-linearity and interaction - clearly defining estimands of interest
...
In Fig 3. we can see that we not only have to take confounding between
the treatment and outcome into account, but we also have to take
...
confounder. This is illustrated in Fig. 4.

Fig. 4. DAG under intermediate confounders of the mediator-outcome
relation affected by treatment
...
traditional regression approaches. If we adjust for Z we open the
backdoor path from $Z \leftarrow W \rightarrow Y$. We will work on how
...
-   Did your study account for all the confunding assumptions? If not,
...
investigate mediation (ref:Imai). One approach is the so-called parallel
design. Two randomized controlled trials are conducted in parallel. The
first investigates the effect of the treatment on the outcome (i.e.,
total effect). In the second trial we randomize both the treatment and
the mediator.
...
called the "ultimatum game" (ref: Knoch 2006). First, you conduct an
experiment with unfair bidding and acceptance. Then one conducts the
same experiment but with manipulation of the mediator, here TMS to
activate a certain brain region.

A key assumption here is consistency. The subjects should not know the
receive the extra intervention.

There is also a crossover version (ref: Imai). Here, we first randomize
the order of the treatment and control, then observe the effect on
mediator and outcome. Then we assign the opposite treatment status and
to the value of the mediator that was observed in the first period.
