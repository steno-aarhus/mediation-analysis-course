...
```{r, echo = FALSE, fig.cap="Basic Mediation Model", out.width = '50%'}
knitr::include_graphics(here::here("images", "mediation_basic.PNG"))
knitr::include_graphics(here::here("images", "mediation_basic2.PNG"))
```



The above shows the standard mediation model. 
...
## Traditional approaches to mediation alaysis
...
- the total effect -->c
- the direct effect --> c', the effect of X on Y after controlling for M
**the product method**
- indirect effect=a*b
...
> Is eating more red meat associated with higher blood glucose by increasing certain inflammation biomarkers?

Based on literature, let's hypothesize that higher red meat intake is associated with elevated inflammatory levels, which, in turn, increase blood glucose.
...
## Path A-Estimate the relationship between X on M 
We then check the path between exposure (red meat) and the mediator (inflammatory biomarker), which corresponds to criteria (i). A mediation makes sense only if X affects M. If X and M have no relationship, M is just a third variable that may or may not be associated with Y.

As the mediator is a continuous variable, we can built a linear regression model, adjusting for confounders on the pathway between X and M. 
...
##Path B- Estimate the relationship between M on Y controlling for X (inflmmatory biomarker on glucose levels, controlling for red meat intake. This step corresponds to criteria (ii). 
...
## Path C-Estimate the relationship between X(red meat intake) on Y (glucose levels). 

Similarly, We conduct a linear regression model adjusting for the confounders. 
...
In our example, 
- the *direct effect* ($\theta_1$) of red meat on glucose levels can be assessed by 
the coefficient of red meat in model b, which is 0.811. 
...
## Difference approach: 
The difference approach is more commonly used in psychological domain. And the rational of this approach is to compare the effect of X on Y without and with adding M in the models. 
...
mediate() takes two model objects as input (X → M and X + M → Y) and we need to specify which variable is an IV (treatment) and a mediator (mediator). For bootstrapping, set boot = TRUE and sims to at least 500. 
...
This is the indirect effect of the IV on the DV that goes through the mediator. Note that ACME estimated to be 0.273, which is the same to the traditional approached we just used.
