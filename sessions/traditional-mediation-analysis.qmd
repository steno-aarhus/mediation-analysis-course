...
a third variable-mediating variable (M). Thus, mediation analysis not
only answers whether two variables are related, but also **why and
how**. This can be visualized in the following figure:

<!--make models as Grviz  -->
...
-   total effect --\>c, that is the total effect of the independent
    variable on the dependent variable
-   direct effect --\> c', that is the effect of the independent
    variable on the dependent variable that is not mediated by the
    mediator
-   mediation effect=a\*b **the product method**
-   mediation effect=c-c' **the difference method**
...
question in nutritional epidemiology using both methods.
...
act through intermediary factors, such as inflammation? Let's imagine
...
Therefore,our interest is know how much the effect of red meat on blood
...
Note that the example is created to demonstrate the methods,and there is
no clinical relevance of the results. For the convenience of analysis, I
have transformed the mediator (m) and outcome variable (y) so that they
...
2.  Tn the model for the outcome that includes both the exposure and
    mediator, the mediator should be associated with the outcome;
3.  Tn the model for the outcome that includes only the exposure, the
...
### Step 1: Estimate the relationship between X on M (path a)
...
biomarker is assocaited with 0.69 unit increase in the blood glucose).
...
results <- mediate(lm_m, lm_y, treat = "a", mediator = "m", boot = TRUE, sims = 500)
